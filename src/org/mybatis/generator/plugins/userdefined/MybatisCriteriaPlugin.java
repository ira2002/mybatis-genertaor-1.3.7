...
public class MybatisCriteriaPlugin extends PluginAdapter {
	...
	public boolean validate(List<String> warnings) {
		...
		if (stringHasValue(properties.getProperty("isAllInOne"))) { //$NON-NLS-1$
			...
			isAllInOne = true;//TODO 是否把所有条件就设置到一个里面
		}
		return true;
	}
	...
	public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
		...
		addSetterComment(method, "\t查询的条件名称\n\t * @param value \t\t查询的值", "condition");
		topLevelClass.addMethod(method);
		...
		addSetterComment(method, "\t排序字段", "orderByClause");
		topLevelClass.addMethod(method);
		method = new Method();
		method.setVisibility(JavaVisibility.PUBLIC);
		...
		addGetterComment(method, "\t排序字段", "orderByClause");
		topLevelClass.addMethod(method);
		...
		addSetterComment(method, "\t是否相异", "distinct");
		topLevelClass.addMethod(method);
		method = new Method();
		method.setVisibility(JavaVisibility.PUBLIC);
		method.setReturnType(new FullyQualifiedJavaType("boolean"));
		method.setName("getDistinct"); //$NON-NLS-1$
		method.addBodyLine("return distinct;"); //$NON-NLS-1$
		addGetterComment(method, "\t是否相异", "distinct");
		topLevelClass.addMethod(method);
		...
		if (databaseType.contains("oracle")) {
			...
			addGetterComment(method, "开始记录数", "oracleStart");
			topLevelClass.addMethod(method);
			...
			addGetterComment(method, "结束记录数", "oracleEnd");
			topLevelClass.addMethod(method);
			...
			addFieldComment(field, "指定返回记录行的偏移量 \\t mysqlOffset= 5,mysqlLength=10;  // 检索记录行 6-15\", \"mysqlOffset");
			topLevelClass.addField(field);
			...
			addSetterComment(method, "\t 指定返回记录行的偏移量 \t mysqlOffset= 5,mysqlLength=10;  // 检索记录行 6-15", "mysqlOffset");
			topLevelClass.addMethod(method);
			method = new Method();
			method.setVisibility(JavaVisibility.PUBLIC);
			method.setReturnType(new FullyQualifiedJavaType("Integer"));
			...
			addGetterComment(method, "\t 指定返回记录行的偏移量 \t mysqlOffset= 5,mysqlLength=10;  // 检索记录行 6-15", "mysqlOffset");
			topLevelClass.addMethod(method);
			...
			addFieldComment(field, "指定返回记录行的最大数目 \\t mysqlOffset= 5,mysqlLength=10;  // 检索记录行 6-15\", \"mysqlOffset");
			topLevelClass.addField(field);
			...
			addSetterComment(method, "\t 指定返回记录行的最大数目 \t mysqlOffset= 5,mysqlLength=10;  // 检索记录行 6-15", "mysqlLength");
			topLevelClass.addMethod(method);
			method = new Method();
			method.setVisibility(JavaVisibility.PUBLIC);
			method.setReturnType(new FullyQualifiedJavaType("Integer"));
			...
			addGetterComment(method, "\t 指定返回记录行的最大数目 \t mysqlOffset= 5,mysqlLength=10;  // 检索记录行 6-15", "mysqlLength");
			topLevelClass.addMethod(method);
		}
		//TODO GeneratedJavaFile构造方法
		//ibatis|mybatis-generator -v 1.2.x	自定义格式
	...
	/**
	 * 	重写clientGenerated方法，将方法的参数列表中entityExample参数替换为Criteria参数
	 */
	@Override
		...
		for (int i = 0; i < methods.size(); i++) {
			...
			if (method.getFormattedContent(0, true).contains("Example")) {
				...
				if (size == 1) {
					...
					method.addParameter(1, parameter2);
				}
				first = false;
			}
		}
	...
	/**
	 * 修改sqlmap.xml中parameterType的值为Criteria
	 */
	@Override
	public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
		...
		// 移除第一个
		Where_Clause.removeElement(0);	//remove Update_By_Example_Where_Clause
...
//				sb.append(MyBatis3FormattingUtilities.getParameterClause(introspectedColumn, "example.condition."));
	...
	private XmlElement getOracleHead() {
		...
		outerisNotEmptyElement.addElement(new TextElement("<![CDATA[ select _row.* from ( select row_.*, rownum rownum_ from ( ]]>"));
	...
	private XmlElement getOracleTail() {
		...
		outerisNotEmptyElement.addElement(new TextElement(
				"<![CDATA[ ) row_ where rownum <= #{oracleEnd} ) _row where rownum_ > #{oracleStart} ]]>"));
		answer.addElement(outerisNotEmptyElement);
		return answer;
	}
